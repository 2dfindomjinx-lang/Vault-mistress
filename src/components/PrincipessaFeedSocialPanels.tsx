  ...
  return <div>
    ...
    {hasQuery && !loading ? <>
      <section className="border-b border-white/10">
        ...
        {accounts.map((account) => <article className="flex items-center justify-between gap-4 border-t border-white/[0.06] px-5 py-4 transition hover:bg-white/[0.025]" key={account.userId}><Link className="min-w-0 transition hover:brightness-125" href={`/principessa-feed/profile/${encodeURIComponent(account.userId)}`}><DisplayNameWithUsername displayName={account.displayName} primaryClassName="font-black" secondaryClassName="text-xs text-zinc-600" secondaryStyle={account.usernameStyle} username={account.username} /></Link>{isLoggedIn ? <MessageLink compact userId={account.userId} /> : null}</article>)}
      ...
      <section>
        ...
        {posts.map((post) => <article className="border-t border-white/[0.06] p-5 transition hover:bg-white/[0.025]" key={post.id}><div className="flex items-start justify-between gap-4"><div>{post.author ? <Link className="block transition hover:brightness-125" href={`/principessa-feed/profile/${encodeURIComponent(post.author.userId)}`}><DisplayNameWithUsername displayName={post.author.displayName} primaryClassName="text-sm font-black" secondaryClassName="text-xs text-zinc-600" secondaryStyle={post.author.usernameStyle} username={post.author.username} /></Link> : <p className="text-sm font-black text-fuchsia-300">Anonymous Confession</p>}<p className="mt-1 text-[10px] font-black uppercase text-pink-300/50">{post.channel} · {post.postType}</p></div>{post.author && isLoggedIn ? <MessageLink compact userId={post.author.userId} /> : null}</div><h3 className="mt-3 font-serif text-xl text-[#ffe4b5]">{post.title}</h3><p className="mt-1 line-clamp-4 whitespace-pre-wrap text-sm leading-6 text-zinc-400">{post.description}</p><p className="mt-3 text-[10px] text-zinc-700">{new Date(post.createdAt).toLocaleString()}</p></article>)}
