...
import { PrincipessaFeedNotifications } from "@/components/PrincipessaFeedNotifications";
import { PrincipessaFeedSocialPanels } from "@/components/PrincipessaFeedSocialPanels";
...
export function PrincipessaSocialFeed({ isAdmin, isLoggedIn }: { isAdmin: boolean; isLoggedIn: boolean }) {
  ...
  return (
    <div className="relative mx-auto grid min-h-[calc(100vh-76px)] max-w-[1380px] grid-cols-1 bg-[#060206] lg:grid-cols-[240px_minmax(0,720px)_300px]">
      ...
      <aside className="hidden border-r border-[#f4c06a]/10 p-5 lg:block"><div className="sticky top-[100px]"><FeedAvatar author={profile ? { avatarUrl: profile.avatarUrl, displayName: profile.displayName, username: profile.username } : undefined} empty large />{profile ? <DisplayNameWithUsername className="mt-5" displayName={profile.displayName} primaryClassName="truncate font-serif text-2xl text-[#ffe5b8]" secondaryClassName="mt-1 truncate text-xs text-zinc-600" username={profile.username} /> : <h1 className="mt-5 truncate font-serif text-2xl text-[#ffe5b8]">{isLoggedIn ? "Loading profile..." : "Guest"}</h1>}<div className="mt-4"><PrincipessaFeedNotifications isLoggedIn={isLoggedIn} /></div><div className="mt-5 grid gap-2"><button className={`rounded-2xl px-4 py-3 text-left font-black ${view === "home" ? "border border-[#f4c06a]/20 bg-pink-500/10 text-[#ffe5b8]" : "text-zinc-500 hover:bg-white/5"}`} onClick={() => setView("home")} type="button">⌂&nbsp;&nbsp; Home</button><button className={`rounded-2xl px-4 py-3 text-left font-black ${view === "profile" ? "border border-[#f4c06a]/20 bg-pink-500/10 text-[#ffe5b8]" : "text-zinc-500 hover:bg-white/5"}`} onClick={openProfile} type="button">♙&nbsp;&nbsp; Profile</button></div><PrincipessaFeedSocialPanels isLoggedIn={isLoggedIn} /><p className="mt-8 border-l border-[#f4c06a]/30 pl-3 font-serif text-sm italic leading-6 text-zinc-600">A velvet room for Principessa’s official visions and approved offerings.</p></div></aside>
      ...
      <section className="min-w-0 border-x border-[#f4c06a]/10 bg-[#090509]/95 shadow-[0_0_50px_rgba(0,0,0,.5)]">
        ...
        <div className="grid gap-2 border-b border-white/10 p-3 lg:hidden"><PrincipessaFeedNotifications isLoggedIn={isLoggedIn} /><PrincipessaFeedSocialPanels isLoggedIn={isLoggedIn} /></div>
