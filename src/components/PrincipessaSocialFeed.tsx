...
export function PrincipessaSocialFeed({ isAdmin, isLoggedIn }: { isAdmin: boolean; isLoggedIn: boolean }) {
  ...
  const loadProfile = useCallback(async () => {
    ...
    finally { profileRequestInFlightRef.current = false; setProfileLoading(false); }
  }, [isLoggedIn]);
  ...
  const openProfile = () => {
    ...
    if (profileRequestInFlightRef.current) setProfileLoading(true);
    else if (!profile && !profileLoading) void loadProfile();
  };
      ...
      <section className="min-w-0 border-x border-[#f4c06a]/10 bg-[#090509]/95 shadow-[0_0_50px_rgba(0,0,0,.5)]">
        ...
        {loading ? <p className="p-16 text-center text-zinc-600">Loading feed...</p> : posts.length === 0 ? <div className="p-16 text-center"><p className="text-lg font-black">No posts yet.</p><p className="mt-2 text-sm text-zinc-600">Published posts will appear here.</p></div> : posts.map((post) => (
          <article className="border-b border-white/10 p-4 transition hover:bg-white/[0.015]" key={post.id}>
            <div className="flex gap-3"><FeedAvatar author={post.author} official={post.channel === "principessa"} /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><p className="font-black">{post.author.displayName}</p><div className="mt-0.5 flex flex-wrap items-center gap-2"><span className="text-xs text-zinc-600">{usernameLabel(post.author.username)}</span><span className={post.channel === "principessa" ? "text-[10px] font-black uppercase text-pink-400" : "text-[10px] font-black uppercase text-zinc-600"}>{post.channel === "principessa" ? "◆ Official court" : "Approved Sub"}</span></div></div><span className="shrink-0 text-[10px] text-zinc-700">{formatDate(post.createdAt)}</span></div><h2 className="mt-2 font-serif text-xl text-[#ffe4b5]">{post.title}</h2><p className="mt-1 whitespace-pre-wrap text-[15px] leading-6 text-zinc-300">{post.description}</p><PostImages post={post} /></div></div>
            {isAdmin ? <PostAdminActions onChanged={setPosts} post={post} /> : null}
            {post.comments.length > 0 ? <div className="ml-14 mt-3 grid gap-3 border-l border-white/10 pl-3">{post.comments.map((item) => <div key={item.id}><div className="flex justify-between gap-2"><div><p className="text-xs font-black text-zinc-300">{item.author.displayName}</p><p className="text-[10px] text-zinc-600">{usernameLabel(item.author.username)}</p></div><span className="text-[10px] text-zinc-700">{formatDate(item.createdAt)}</span></div><p className="mt-1 text-sm text-zinc-400">{item.body}</p></div>)}</div> : null}
