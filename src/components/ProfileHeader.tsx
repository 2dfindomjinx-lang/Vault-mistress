...
export function ProfileHeader({
  ...
  if (compact) return (
    <header className="court-account-bar relative flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#c89a55]/20 bg-[#13090f] px-4 py-3">
      <div className="min-w-0">
        <p className="truncate font-serif text-lg text-[#fff0d2]" style={usernameStyle}>{visibleName}</p>
        <p className="text-xs text-zinc-400">{currentTitle}</p>
      </div>
      <div className="flex flex-wrap gap-4">
        {ledgerStats.slice(0, 3).map(stat => (
          <div key={stat.label}>
            <p className="text-[11px] text-zinc-400">{stat.label}</p>
            <div className="text-sm font-semibold text-[#fff0d2]">{stat.value}</div>
          </div>
        ))}
      </div>
      <div className="flex w-full flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-3">
