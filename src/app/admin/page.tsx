...
export default function AdminPage() {
  ...
  const deleteJigsawLink = (id: string) =>
    submitJigsawAction({ action: "delete", id }, "Jigsaw link deleted.");
    ...
    <main className="min-h-screen bg-[#08070b] px-3 py-4 text-white sm:px-5 lg:px-6">
      <section className="mx-auto max-w-[94rem] rounded-xl border border-white/10 bg-[#0d0a12] shadow-[0_18px_60px_rgba(0,0,0,0.35)]">
        ...
        <div className="min-w-0 px-4 pb-4 pt-3">
          ...
          {activeTab === "jigsaw" && (
            <div className="mt-4 rounded-[1.5rem] border border-sky-200/20 bg-[#050208] p-4 shadow-[inset_0_0_24px_rgba(56,189,248,0.08)]">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-xs uppercase tracking-[0.24em] text-sky-200/70">Jigsaw Pool</p>
                  <p className="mt-1 text-xs text-zinc-500">
                    Subs pay coins to have one of these picked for them at random. There is no completion
                    check and no reward - the spend is the whole transaction. URLs must be https.
                  </p>
                </div>
              ...
              <div className="mt-4 space-y-2">
                {jigsawLinks.length === 0 ? (
                  <p className="text-xs text-zinc-500">No jigsaws yet. Add one below.</p>
                ) : (
                  jigsawLinks.map((entry) => (
                    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3" key={entry.id}>
                      <input
                        className="w-full rounded-xl border border-white/10 bg-black/45 px-3 py-2 text-sm text-white"
                        onChange={(event) => setJigsawLinks((links) => links.map((item) => item.id === entry.id ? { ...item, label: event.target.value } : item))}
                        placeholder="Label"
                        value={entry.label}
                      />
                      <input
                        className="mt-2 w-full rounded-xl border border-white/10 bg-black/45 px-3 py-2 text-sm text-white"
                        onChange={(event) => setJigsawLinks((links) => links.map((item) => item.id === entry.id ? { ...item, url: event.target.value } : item))}
                        placeholder="https://..."
                        value={entry.url}
                      />
                      <div className="mt-2 flex flex-wrap items-center gap-3">
                        <label className="flex flex-col gap-1 text-[10px] uppercase tracking-wide text-zinc-500">
                          ...
                          <input
                            className="w-32 rounded-xl border border-white/10 bg-black/45 px-3 py-2 text-sm text-white normal-case tracking-normal"
                            min={0}
                            onChange={(event) => setJigsawLinks((links) => links.map((item) => item.id === entry.id ? { ...item, coin_cost: Number(event.target.value) } : item))}
                            type="number"
                            value={entry.coin_cost}
                          />
                        </label>
                        <label className="flex items-center gap-2 text-xs text-zinc-300">
                          <input
                            checked={entry.enabled}
                            onChange={(event) => setJigsawLinks((links) => links.map((item) => item.id === entry.id ? { ...item, enabled: event.target.checked } : item))}
                            type="checkbox"
                          />
                          Enabled
                        </label>
                        <button className="rounded-xl border border-sky-100/30 bg-sky-300/15 px-3 py-1.5 text-xs font-bold text-sky-50 disabled:opacity-50" disabled={isBusy} onClick={() => void saveJigsawLink(entry)} type="button">Save</button>
                        <button className="rounded-xl border border-red-200/30 bg-red-400/10 px-3 py-1.5 text-xs font-bold text-red-100 disabled:opacity-50" disabled={isBusy} onClick={() => void deleteJigsawLink(entry.id)} type="button">Delete</button>
                      </div>
                    </div>
                  ))
                )}
              ...
              <div className="mt-4 space-y-2 rounded-2xl border border-white/10 bg-white/[0.03] p-3">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <input className="w-32 rounded-xl border border-white/10 bg-black/45 px-3 py-2 text-sm text-white" min={0} onChange={(event) => setJigsawForm((form) => ({ ...form, coinCost: event.target.value }))} type="number" value={jigsawForm.coinCost} />
                  <button
                    className="rounded-xl border border-sky-100/30 bg-sky-300/15 px-4 py-2 text-sm font-bold text-sky-50 disabled:cursor-not-allowed disabled:opacity-50"
                    disabled={isBusy || !jigsawForm.label.trim() || !jigsawForm.url.trim()}
                    onClick={() => void addJigsawLink()}
                    type="button"
                  >
                    Add
                  </button>
