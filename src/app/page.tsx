...
import { getTimeoutClearFee, PET_FAVOR_ROULETTE_COIN_REWARD, PET_TASK_COIN_REWARD, roundRewardToNearestFive, TIMEOUT_CLEAR_FEE_PER_HOUR } from "@/lib/server-game-rules";
import {
...
const eventPetTaskCoinReward = getEventTaskReward(PET_TASK_COIN_REWARD);
  ...
  const handleClickGameToggle = async (action: "start" | "stop" | "reset") => {
    ...
    try {
      const response = await fetch(`/api/user/click-game/${action}`, {
        body: JSON.stringify({ category: clickGameCategory }),
        headers: { "Content-Type": "application/json" },
        method: "POST",
      });
      const payload = (await response.json().catch(() => null)) as { error?: string; status?: ClickGameStatus } | null;
  ...
  const handleCaseOpen = async () => {
    if (blockIfTimedOut()) throw new Error("Timeout is active.");
    const actionId = "case-opening";
    const task = tasks.find((entry) => entry.id === actionId);
    if (!task || (task.cooldownUntil && new Date(task.cooldownUntil).getTime() > Date.now())) {
      throw new Error("Case Opening is on cooldown until the daily reset.");
    }
    if (!beginTaskAction(actionId)) return null;
    try {
      let reward: number;
      let openedAt: string;
      let cooldownUntil: string | null;
      if (isGuestMode) {
        reward = randomCaseOpeningReward();
        openedAt = new Date().toISOString();
        cooldownUntil = getPetTaskCooldownUntil(openedAt);
        coinsRef.current += reward;
        setCoins(coinsRef.current);
      } else {
        const response = await fetch("/api/user/task-actions/case-opening", { method: "POST" });
        const result = await response.json() as { error?: string; reward?: number; coins?: number; openedAt?: string; cooldownUntil?: string };
        if (!response.ok || !Number.isFinite(result.reward) || typeof result.coins !== "number" || !result.openedAt || !result.cooldownUntil) {
          if (result.cooldownUntil) setTasks((current) => current.map((entry) => entry.id === actionId ? { ...entry, cooldownUntil: result.cooldownUntil } : entry));
          throw new Error(result.error ?? "Case Opening could not be saved. Please try again.");
        }
      ...
      setTasks((current) => current.map((entry) => entry.id === actionId ? {
        ...entry, caseReward: reward, caseSpunAt: openedAt, completed: true, claimed: true, cooldownUntil,
      } : entry));
      return reward;
    } finally {
      finishTaskAction(actionId);
