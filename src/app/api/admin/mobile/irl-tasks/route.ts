...
export async function POST(request: Request) {
  ...
  if (body.action === "approve") {
    ...
    await admin.supabase
      .from("profiles")
      .update({ affection: Math.min(100, Number(profile.affection ?? 0) + IRL_TASK_APPROVAL_AFFECTION_GAIN), updated_at: now })
      .eq("id", profile.id);
  ...
  if (body.action === "cancelShame") {
    ...
    const { data: profile } = await admin.supabase.from("profiles").select("id, shame_count").eq("id", task.user_id).maybeSingle();
    await admin.supabase
      .from("profiles")
      .update({ shame_count: Number(profile?.shame_count ?? 0) + 1, timeout_until: timeoutUntil, updated_at: now })
      .eq("id", task.user_id);
    const { error } = await admin.supabase
      .from("user_irl_tasks")
