...
create table if not exists public.app_activation_codes (
  ...
  max_resets integer not null default 2,
  created_at timestamp with time zone not null default now(),
  updated_at timestamp with time zone not null default now()
);
...
create table if not exists public.app_activation_events (
  ...
  event_type text not null check (event_type in ('generated', 'activated', 'validated', 'rejected', 'reset', 'revoked')),
...
alter table public.app_activation_codes
  ...
  add column if not exists max_resets integer not null default 2,
  add column if not exists updated_at timestamp with time zone not null default now();
