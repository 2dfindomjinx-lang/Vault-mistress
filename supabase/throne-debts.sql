...
alter table public.throne_debts
  add column if not exists debt_code text,
...
set debt_code = 'TD-' || upper(substr(replace(id::text, '-', ''), 1, 8))
where debt_code is null;
create unique index if not exists idx_throne_debts_debt_code on public.throne_debts (debt_code) where debt_code is not null;
